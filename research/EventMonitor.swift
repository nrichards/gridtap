...
public final class EventMonitor {
    ...
    /// get called.
    ///
    public func handle(_ eventTypes: Set<CGEventType>, with fn: @escaping (CGEvent) -> ()) {
    ...
    /// get called.
    ///
    public func handle(_ eventType: CGEventType, with fn: @escaping (CGEvent) -> ()) {
    ...
    ///
    public func handleKeyDown(with fn: @escaping (String, CGEventFlags) -> ()) {
    ...
    public func start(on runLoop: RunLoop = .current) throws {
        ...
        guard let port = CGEvent.tapCreate(
            ...
            callback: { (_ proxy: CGEventTapProxy, _ eventType: CGEventType, _ event: CGEvent, _ userInfo: UnsafeMutableRawPointer?) -> Unmanaged<CGEvent>? in
    ...
    private var handlers: [(Set<CGEventType>, (CGEvent) -> ())] = []
    private var port: CFMachPort? = nil
}
